...
export function VideoUpload({
  ...
  const countOrUndefined = expectedCount ?? undefined
  ...
  return (
    <div className="rounded-lg border border-dashed bg-card p-8 text-center">
      ...
      <label className="mt-6 inline-flex flex-wrap items-center justify-center gap-2 text-sm">
        ...
        <input
          ...
          onChange={(event) => {
            ...
            setExpectedCount(Math.max(1, Math.min(40, Number(raw) || 1)))
          }}
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
        ...
        <Button
          ...
          variant="outline"
          disabled={disabled}
          onClick={() => onSample(countOrUndefined)}
        >
          Use demo sample
        </Button>
      </div>
      <input
        ...
        onChange={(event) => {
          ...
          if (file) onUpload(file, countOrUndefined)
          event.target.value = ''
        }}
      />
    </div>
